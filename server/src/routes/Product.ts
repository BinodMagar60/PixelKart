...
} from "../utils/RandomOrderGenerator";
...
router.get("/review/:productId", async (req, res) => {
  try {
    ...
    const allReivew = await Review.find({ productId: productId }).sort({updatedAt: -1}).select("-__v -updatedAt");
    
    const filteredData = await Promise.all(allReivew.map(async(item) => {
      const userData = await User.findOne({_id: item.reviewerId})
      return {
        id: item._id,
        rating: item.reviewStar,
        name: userData?.firstName + " "+ userData?.secondName,
        description: item.reviewComment,
      };
    }))

    res
      .status(200)
      .json({ message: "Product Review received", data: filteredData });
  } catch (error) {
    res.status(500).json({ message: "Server error", error });
  }
});
...
router.delete("/cart", async (req, res) => {
  try {
    ...
    if (!existingOrder) {
      res
        .status(400)
        .json({ message: "Cart item not found. Please reload the page" });
      return;
    }
    ...
    const newData = await Promise.all(
      data.map(async (item) => {
        ...
        if (!productExist) {
          res
            .status(400)
            .json({ message: "One or more product might not be available" });
          return;
        }
        if (item.orderQty > productExist.qty) {
          res
            .status(400)
            .json({
              message: "The product quantity is greater than available product",
            });
          return;
        }
        return {
